...
export function CartDrawer({ open, onClose }: Props) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className={`fixed top-0 right-0 h-full w-full max-w-sm bg-white dark:bg-lumina-taupe-900 shadow-2xl z-50 flex flex-col transition-transform duration-300 ease-in-out ${
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-center">
              ...
              <button
                onClick={onClose}
                className="mt-2 px-6 py-2.5 rounded-full btn-brand text-sm font-semibold transition"
              >
                {t.cart.browse}
              </button>
            </div>
        ...
        {items.length > 0 && (
          <div className="px-6 py-5 border-t border-gray-100 dark:border-white/10 space-y-4">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-6 py-4 rounded-xl btn-brand font-semibold hover:scale-[1.02] transition-all disabled:opacity-70 disabled:cursor-not-allowed disabled:scale-100"
            >
              {checkingOut ? (
                <>
