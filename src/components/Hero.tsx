...
export function Hero() {
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden">
      {/* Background */}
      <div className="absolute inset-0 bg-gradient-to-br dark:from-surface dark:via-[#1c1710] dark:to-deep" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 flex flex-col lg:flex-row items-center gap-16">
        ...
        <div className="flex-1 text-center lg:text-left">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start animate-slide-up" style={{ animationDelay: "0.2s" }}>
            ...
            <a
              href="#reviews"
              className="flex items-center justify-center gap-2 px-8 py-4 rounded-full border border-white/10 bg-white/5 text-gray-200 font-semibold text-lg hover:bg-white/10 transition-all"
            >
              See Reviews
            </a>
          </div>
    ...
    >
      ...
      <div className="bg-card p-4">
        <div className="flex items-center justify-between mb-1">
          ...
          {badge && (
            <span className="text-xs px-2 py-0.5 rounded-full bg-brand-500/10 text-brand-400 font-medium ml-1 shrink-0">
              {badge}
            </span>
          )}
        </div>
        <p className="text-brand-400 font-bold text-lg">{price}</p>
      </div>
    </div>
  );
}
